...
#ifdef HAVE_PCP
/*
 * Display only datafile header for PCP archive imput.
 */
struct report_format hdr_fmt_pcp = {
	.id		= F_HEADER_OUTPUT,
	.options	= FO_HEADER_ONLY,
	.f_header	= print_hdr_header_pcp,
	.f_statistics	= NULL,
	.f_timestamp	= NULL,
	.f_restart	= NULL,
	.f_comment	= NULL,
...
#ifdef HAVE_PCP
/*
 * Database friendly format for PCP archive input.
 */
struct report_format db_fmt_pcp = {
	.id		= F_DB_OUTPUT,
	.options	= FO_LOCAL_TIME + FO_HORIZONTALLY +
			  FO_SEC_EPOCH + FO_FIELD_LIST,
	.f_header	= NULL,
...
#ifdef HAVE_PCP
/*
 * Format easily handled by pattern processing commands like awk for PCP archive input.
 */
struct report_format ppc_fmt_pcp = {
	.id		= F_PPC_OUTPUT,
	.options	= FO_LOCAL_TIME + FO_SEC_EPOCH,
	.f_header	= NULL,
	.f_statistics	= NULL,
...
#ifdef HAVE_PCP
/*
 * XML output for PCP archive input.
 */
struct report_format xml_fmt_pcp = {
	.id		= F_XML_OUTPUT,
	.options	= FO_HEADER_ONLY + FO_LOCAL_TIME + FO_TEST_MARKUP,
	.f_header	= print_xml_header,
	.f_statistics	= print_xml_statistics,
...
#ifdef HAVE_PCP
/*
 * JSON output for PCP archive input.
 */
struct report_format json_fmt_pcp = {
	.id		= F_JSON_OUTPUT,
	.options	= FO_HEADER_ONLY + FO_LOCAL_TIME + FO_TEST_MARKUP +
			  FO_LC_NUMERIC_C,
	.f_header	= print_json_header,
...
#ifdef HAVE_PCP
/*
 * Convert an old datafile to up-to-date format for PCP archive input.
 */
struct report_format conv_fmt_pcp = {
	.id		= F_CONV_OUTPUT,
	.options	= 0,
	.f_header	= NULL,
	.f_statistics	= NULL,
...
#ifdef HAVE_PCP
/*
 * SVG output for PCP archive input.
 */
struct report_format svg_fmt_pcp = {
	.id		= F_SVG_OUTPUT,
	.options	= FO_HEADER_ONLY + FO_LOCAL_TIME +
			  FO_LC_NUMERIC_C,
	.f_header	= print_svg_header,
...
#ifdef HAVE_PCP
/*
 * Raw output for PCP archive input.
 */
struct report_format raw_fmt_pcp = {
	.id		= F_RAW_OUTPUT,
	.options	= FO_LOCAL_TIME + FO_SEC_EPOCH,
	.f_header	= NULL,
	.f_statistics	= NULL,
...
/*
 * PCP output for PCP archive input.
 */
struct report_format pcp_fmt_pcp = {
	.id		= F_PCP_OUTPUT,
	.options	= FO_HEADER_ONLY + FO_LOCAL_TIME + FO_NO_TRUE_TIME +
			  FO_ITEM_LIST + FO_FULL_ORDER,
	.f_header	= print_pcp_header,
...
#ifdef HAVE_PCP
/*
 * Array of output formats for PCP input archive.
 */
struct report_format *fmt_pcp[NR_FMT] = {
	&hdr_fmt_pcp,
	&db_fmt_pcp,
	&ppc_fmt_pcp,
	&xml_fmt_pcp,
